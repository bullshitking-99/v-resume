...
export function BlogCard({ title, url, period, techStack, posts, className }: BlogCardProps) {
  return (
      ...
      <ul className="text-sm space-y-1">
        {posts.map((post, index) => (
          <li key={index} className="pl-4 relative">
            ...
            {post.url ? (
              <a href={post.url} target="_blank" className="hover:underline text-muted-foreground" rel="noreferrer">
                {post.title}
              </a>
            ) : (
